...
def get_json(url, i, headersList, payload):
    ...
    ret = requests.request("GET", url.format(
        i), data=payload, headers=headersList)
...
while len(json_data['search']['result']['listings']) > 0:
    ...
    for i in range(0, qtd):
        ...
        try:
            try:
                endereco = json_data['search']['result']['listings'][i]['listing']['address']['street'] + \
                    ", " + \
                    json_data['search']['result']['listings'][i]['listing']['address']['streetNumber']
            except:
                endereco = json_data['search']['result']['listings'][i]['listing']['address']['street']
        except:
            endereco = '-'
        ...
        try:
            ...
            condominio = '-'
        try:
            wlink = 'https://www.vivareal.com.br/' + \
